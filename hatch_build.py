...
`src/samplescope/web_dist/`, which the wheel picks up via the `artifacts`
config. So `uv tool install samplescope` (or `uvx`) ships a fully
...
Set SAMPLESCOPE_SKIP_WEB_BUILD=1 to reuse an existing web_dist/ (useful
...
class WebDistBuildHook(BuildHookInterface):
    ...
    def initialize(self, version: str, build_data: dict) -> None:
        ...
        if os.environ.get("SAMPLESCOPE_SKIP_WEB_BUILD") == "1":
            if not (staged / "index.html").exists():
                raise RuntimeError(
                    "SAMPLESCOPE_SKIP_WEB_BUILD=1 but no existing "
        ...
        if shutil.which("npm") is None:
            raise RuntimeError(
                "npm is required to build the samplescope wheel "
                "(it compiles the web UI). Install node, or set "
                "SAMPLESCOPE_SKIP_WEB_BUILD=1 to reuse a previous build."
            )
